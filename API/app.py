...
try:
     os.path.isfile(path+'/Data/file/ip.json')
     print ("File exist")
except ValueError:
...
def tweet_entity():
    setIP()
    with open(path+"/Data/file/entity.json") as jsonFile:
...
def form_example():
    dr = pd.read_csv(path+"/Data/file/tweet.csv")
...
def json_example():
    dr = pd.read_csv(path+"/Data/file/tweet_clean.csv")
    resp = make_response(dr.to_csv())
...
def get_my_ip():
    setIP()
    with open(path+"/Data/file/ip.json") as jsonFile:   
